...
(function (root, factory) {
  ...
  QueryBuilder.prototype.createRuleInput = function (rule) {
    ...
    for (var i = 0; i < rule.operator.nb_inputs; i++) {
      ...
      //-------InsertIconIntoTextInTextBoxCalendar--------------------------------------------------------***************************************Start
      if (boolCalendar) {
        let TmpIdTxt = $ruleInput[0].id;
        let TmpDivSpan = $ruleInput[0].id.split("_")[0];
        if (TmpDivSpan == "divSpan")
          TmpIdTxt = TmpIdTxt.replace(TmpDivSpan + "_", "");

        let glyphiconCalendar = document.createElement("span");
        glyphiconCalendar.className =
          "glyphicon glyphicon-calendar glyphiconCalendar";
        glyphiconCalendar.setAttribute("id", "Icon" + TmpIdTxt);
        glyphiconCalendar.setAttribute("name", "Icon" + TmpIdTxt);
        glyphiconCalendar.setAttribute("data-mddatetimepicker", true);
        glyphiconCalendar.setAttribute("data-targetselector", "#" + TmpIdTxt);
        glyphiconCalendar.setAttribute("data-enabletimepicker", false);
        glyphiconCalendar.setAttribute("data-placement", "right");
        glyphiconCalendar.addEventListener("click", (e) => {
          $("#" + e.target.id).MdPersianDateTimePicker({
            targetTextSelector: "#" + TmpIdTxt,
            targetDateSelector: "#" + TmpIdTxt,
          });
        });
        let txtDiv = document.getElementById($ruleInput[0].id);
        txtDiv.appendChild(glyphiconCalendar);
      }
      //-------InsertIconIntoTextInTextBoxCalendar--------------------------------------------------------***************************************End
    ...
    // $('#' + tmpName).appendChild(glyphiconCalendar)
  };
...
<div id="{{= it.rule_id }}" class="rule-container"> \
  ...
  QueryBuilder.templates.filterSelect =
    '\
{{ var optgroup = null; }} \
<select class="form-control selectComboReportFilter" id="filter_{{= it.rule.id }}"  name="{{= it.rule.id }}_filter"> \
...
{{?}} \
{{ var optgroup = null; }} \
<select class="selectComboReportOperator form-control {{? it.operators.length === 1 }}hide{{?}}" data-live-search="true" name="{{= it.rule.id }}_operator"> \
  ...
  QueryBuilder.templates.ruleValueSelect =
    '\
{{ var optgroup = null; }} \
<select class="selectpicker selectString form-control " data-live-search="true" name="{{= it.name }}" {{? it.rule.filter.multiple }}multiple{{?}}> \
  ...
  function removeDateIcon(name) {
    try {
      let parent = document.getElementById("divSpan_" + name);
      parent.remove();
    } catch (error) {}
  }
      ...
      switch (filter.input) {
        ...
        default:
          if (
            ...
            // ToDo
            h += '<div id="divSpan_' + name + '" class="divCalendar">';
            h +=
              '<input id="' +
              name +
              '" type="text" name="' +
              name +
              `" class="txtCalendar" placeholder= ${reportResources.get(
                "date"
              )}/>`;
            ArrCalendar.push({
              txtId: name,
            ...
            h += '<input id="' + name + '" type="text" name="' + name + '"';
            if (placeholder) h += ' placeholder="' + placeholder + '"';
...
<div" +
  ...
  QueryBuilder.extend(
    /** @lends module:plugins.SqlSupport.prototype */ {
      ...
      getSQL: function (stmt, nl, data) {
        ...
        if (!data) {
          let tmpRootId = this.model.root.id;
          const ruleValueContainer = document.querySelector(
            `#${tmpRootId} .rule-value-container`
          );
          if (
            ruleValueContainer &&
            ruleValueContainer.classList.contains("rule-value-container")
          ) {
            // console.log("کلاس rule-value-container وجود دارد");
            return null;
          } else {
            // console.log("کلاس rule-value-container وجود ندارد");
            return false;
          }
        }
            ...
            if (rule.rules && rule.rules.length > 0) {
              ...
              if (ope.nb_inputs !== 0) {
                ...
                rule.value.forEach(function (v, i) {
                  ...
                  //-------------------------------------------------------------------------------------------------------************************************************SingleQoutionDate&StringType
                  if (
                    ((rule.type == "string" ||
                      rule.type == "localstring" ||
                      rule.type == "latinstring" ||
                      rule.type == "text") &&
                      !(
                    ...
                    let tmpTypeValue = typeof v;
                    if (v != undefined) {
                      if (tmpTypeValue == "string") {
                        if (v.includes("'")) {
                          let validationParams = v.replace(/'/g, "\\'");
                          v = validationParams;
                        }
                        v = "N'" + v + "'";
                      } else if (tmpTypeValue == "object") {
                        $.each(v, function (indexItem, tmpData) {
                          v[indexItem] = "'" + tmpData + "'";
                        });
                      }
                    }
  ...
  //  between: "موجود بین",
  //  not_between: "نا موجود بین",
  //  begins_with: "شروع شامل باشد",
  //  not_begins_with: "شروع شامل نباشد",
  //  contains: "حاوی",
  //  not_contains: "نا حاوی",
  //  ends_with: "پایان شامل باشد",
  //  not_ends_with: "پایان شامل نباشد",
  //  is_empty: "خالی",
  //  is_not_empty: "پر",
  //  is_null: "مقدار خالی",
  //  is_not_null: "مقدار پر",
  //},
  //errors: {
  //  no_filter: "فیلتر انتخاب نشده",
  //  empty_group: "گروه خالی می باشد",
  //  radio_empty: "هیچ مقداری انتخاب نشده",
  //  checkbox_empty: "هیچ مقداری انتخاب نشده",
  //  select_empty: "هیچ مقداری انتخاب نشده",
  //  string_empty: "مقدار خالی است",
  //  string_exceed_min_length: "باید حداقل {0} نویسه داشته باشد",
  //    string_exceed_max_length: "نباید بیش از {0} نویسه داشته باشد",
  //    string_invalid_format: "({0})قالب نامعتبر",
  //    number_nan: "عدد نباید باشد",
  //    number_not_integer: "عدد نباید صحیح باشد",
  //    number_not_double: "عدد واقعی نیست",
  //    number_exceed_min: "باید بزرگتر از {0} باشد",
  //    number_exceed_max: "باید کوچکتر از {0} باشد",
  //    number_wrong_step: "باید مضرب {0} باشد",
  //    number_between_invalid: "مقادیر نامعتبر ، {0} بزرگتر از {1} است",
  //    datetime_empty: "مقدار خالی",
  //    datetime_invalid: " ({0})قالب تاریخ نامعتبر",
  //    datetime_exceed_min: "باید بعد از {0} باشد",
  //    datetime_exceed_max: "باید قبل از {0} باشد",
  //    datetime_between_invalid: "مقادیر نامعتبر ، {0} بزرگتر از {1} است",
  //    boolean_not_valid: "بولین نیست",
  //    operator_not_multiple: 'عملگر "{1}"نمی تواند چندین مقدار را بگیرد ',
  //  },
  //  invert: "Invert",
  //  NOT: "NOT",
  //};
  ...
  QueryBuilder.regional["en"] = {
    __locale: "English (en)",
    __author: 'Damien "Mistic" Sorel, http://www.strangeplanet.fr',
    add_rule: reportResources.get("addcondi"),
    add_group: reportResources.get("addgroup"),
    delete_rule: " ",
    delete_group: " ",
    conditions: {
      AND: "&nbsp;&nbsp;" + reportResources.get("and") + "&nbsp;&nbsp;",
      OR: "&nbsp;&nbsp;" + reportResources.get("or") + "&nbsp;&nbsp;",
    },

    operators: {
      equal: reportResources.get("equal"),
      not_equal: reportResources.get("not_equal"),
      in: reportResources.get("in"),
      not_in: reportResources.get("not_in"),
      less: reportResources.get("less"),
      less_or_equal: reportResources.get("less_or_equal"),
      greater: reportResources.get("greater"),
      greater_or_equal: reportResources.get("greater_or_equal"),
      between: reportResources.get("between"),
      not_between: reportResources.get("not_between"),
      begins_with: reportResources.get("begins_with"),
      not_begins_with: reportResources.get("not_begins_with"),
      contains: reportResources.get("contains"),
      not_contains: reportResources.get("not_contains"),
      ends_with: reportResources.get("ends_with"),
      not_ends_with: reportResources.get("not_ends_with"),
      is_empty: reportResources.get("is_empty"),
      is_not_empty: reportResources.get("is_not_empty"),
      is_null: reportResources.get("is_null"),
      is_not_null: reportResources.get("is_not_null"),
    },
    errors: {
      no_filter: reportResources.get("no_filter"),
      empty_group: reportResources.get("empty_group"),
      radio_empty: reportResources.get("radio_empty"),
      checkbox_empty: reportResources.get("checkbox_empty"),
      select_empty: reportResources.get("select_empty"),
      string_empty: reportResources.get("string_empty"),
      string_exceed_min_length: reportResources.get("string_exceed_min_length"),
      string_exceed_max_length: reportResources.get("string_exceed_max_length"),
      string_invalid_format: reportResources.get("string_invalid_format"),
      number_nan: reportResources.get("number_nan"),
      number_not_integer: reportResources.get("number_not_integer"),
      number_not_double: reportResources.get("number_not_double"),
      number_exceed_min: reportResources.get("number_exceed_min"),
      number_exceed_max: reportResources.get("number_exceed_max"),
      number_wrong_step: reportResources.get("number_wrong_step"),
      number_between_invalid: reportResources.get("number_between_invalid"),
      datetime_empty: reportResources.get("datetime_empty"),
      datetime_invalid: reportResources.get("datetime_invalid"),
      datetime_exceed_min: reportResources.get("datetime_exceed_min"),
      datetime_exceed_max: reportResources.get("datetime_exceed_max"),
      datetime_between_invalid: reportResources.get("datetime_between_invalid"),
      boolean_not_valid: reportResources.get("boolean_not_valid"),
      operator_not_multiple: reportResources.get("operator_not_multiple"),
    },
    invert: reportResources.get("invert"),
    NOT: reportResources.get("not"),
  };
  /*QueryBuilder.regional['en'] = {
  "__locale": "English (en)",
  "__author": "Damien \"Mistic\" Sorel, http://www.strangeplanet.fr",
