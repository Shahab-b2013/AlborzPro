...
(function ($) {
    ...
    function parsePreviousDateTimeValue(persianDateTimeString) {
        ...
        if (previousDateTime.indexOf(':') > 0) { // بدست آوردن مقادیر ساعت و مقدار دهی آنها
            ...
            hour = Number(previousDateTime.match(/\d{1,2}(?=:)(?!:\d{1,2}:)/im));
            minute = Number(previousDateTime.match(/:(\d+$)/im)[1]);
            second = 0;
        }
    ...
    function createDateTimePickerHtml($popoverDescriber, dateTimeInJsonFormat, writeDateString, initializing) {
        var triggerName = $popoverDescriber.attr('data-trigger'),
            ...
            $calendarTimePicker = $('<tr><td colspan="100" style="padding: 2px;"><table class="table" data-name="Md-PersianDateTimePicker-TimePicker"><tr><td>ساعت : <input type="text" class="form-control" data-name="Clock-Hour" maxlength="2" /></td><td><b style="font-size:15px">:</b></td><td>دقیقه : <input type="text" class="form-control" data-name="Clock-Minute" maxlength="2" /></td><td style="display:none">:</td><td><input type="text" class="form-control" data-name="Clock-Second" maxlength="2" style="display:none" /></td></tr></table></td></tr>'),
