  ...
  $.each(sqlOperators, function (index, data) {
    switch (data) {
      case "ExpressionGroupBy":
        txtLabel = dashResources.get("value");
        break;
      case "Sum":
        txtLabel = dashResources.get("sum");
        break;
      case "Avg":
        txtLabel = dashResources.get("avg");
        break;
      case "Min":
        txtLabel = dashResources.get("min");
        break;
      case "Max":
        txtLabel = dashResources.get("max");
        break;
      case "Count":
        txtLabel = dashResources.get("count");
        break;
...
function refreshFilterItems(filterTarget) {
  if (OBJECT_ARRAY.length >= 1) {
    ...
    if (TmpAddValue != "" || TmpAddValue != null) {
      ...
      //نمایش
      fillAfterChangeFilterArray("", filterTarget);
    }
  }
}
...
function addFilterItem(ColumnVal) {
  ...
  if (OBJECT_ARRAY.length > 0) {
    ...
    if (tmpIdNumber != "" || tmpIdNumber != null) {
      ...
      if (tmpEmptyElement != undefined && tmpEmptyElement.value == "-1") {
        ...
        if (tmplastValue == ColumnVal) {
          ...
          if (CheackItemPrev && TmpValueFilterPrev != "-1") {
            for (let index = 0; index < REF_FILTER_DATA.length; index++) {
              ...
              if (REF_FILTER_DATA[index].Name == TmpValueFilterPrev) {
                filterArr.unshift({
                  id: REF_FILTER_DATA[index].Name,
                  idCol: REF_FILTER_DATA[index].ID,
                  name: REF_FILTER_DATA[index].Name,
    ...
    if (TmpValueFilterPrev != "") {
      for (const i in filterArr) {
        //اگ مقدار آخری از قبل توی آرایه فیلتر وجود داشت، نمی خواد اضافش کنی
        if (filterArr[i].id == TmpValueFilterPrev) CheackItemPrev = false;

        //اگرم مقدار حذف شده از قبل توی آرایه فیلتر وجود داشت، نمی خواد اضافش کنی
        if (filterArr[i].id == ColumnVal) CheackItemNow = false;
      }
      if (CheackItemPrev && TmpValueFilterPrev != "-1") {
        for (let index = 0; index < REF_FILTER_DATA.length; index++) {
          //بیا مقدار آخری رو جستجو کن و به اول آرایه فیلتر اضافش کن
...
function visibleSqlSelects() {
  ...
  if (TmpValueFilterNow == "-1" || TmpValueFilterNow == undefined) {
...
function getSql() {
  ...
  for (let i = 0; i < sortypeVal.length; i++) {
    ...
    if (
      (sortypeVal[i].operatorVal != "unsorted" && IsGroupByCount > 0) ||
      (sortypeVal[i].operatorVal != "unsorted" && IsExpression)
    ) {
      if (!firstSort) {
        sortypeStr += ", ";
      }
  ...
  if (coulumnStr != null) coulumnStr = coulumnStr.replace(/-/g, ".");
...
function getFilterPage() {
  if ($(`.rule-container`).length) {
    ...
    if (!$.isEmptyObject(result)) {
      ...
      JsonData.Filter = OUTPUT[0].Filter = result;
    }
    var SqlResult = $("#builder-basic").queryBuilder("getSQL", "question_mark");
    if (SqlResult.sql != null) MyWhereResult = SqlResult.sql;
    if (SqlResult.params != null) {
...
function nextTab(divId) {
  if ($(`#Reportcontent .rowContent`).length == 1) {
    if (divId == "btn_rpt-wiz-2") {
      if ($(`#Reportcontent .rowContent .selectCombo`).val() != -1) {
        next(divId);
      }
    } else {
      next(divId);
    }
  } else if ($(`#Reportcontent .rowContent`).length > 0) {
    next(divId);
  }
}

function categoryWizard(state) {
  //Categroy From Query
  $CategoryQuery = [];
  //when function calls from wizard category label gets empty to prevent duplicated labels
  if (state == "wizard") $categorylabel = [];

  if (!$OBJECT_ARRAY.length) {
    for (let i = 0; i < $categorylabelFDM.length; i++) {
      $CategoryQuery.push({ [i]: $categorylabelFDM[i] });
    }
  } else {
    for (let i = 0; i < $OBJECT_ARRAY.length; i++) {
      let id = $OBJECT_ARRAY[i].filterVal; //Gets column names from object array to set as key
      let value;
      if (state == "wizard") {
        //Extract lables from wizard modal dropdown
        value =
          $OBJECT_ARRAY[i].CommandVal != "ExpressionGroupBy"
            ? $(
                `#` + $OBJECT_ARRAY[i].commandTarget + " option:selected"
              ).text() +
              "" +
              $(`#` + $OBJECT_ARRAY[i].filterTarget + " option:selected").text()
            : $(
                `#` + $OBJECT_ARRAY[i].filterTarget + " option:selected"
              ).text();
        //Push labels to categorylabe array to save and use then in Chartquery
        $categorylabel.push({ [i]: value });
        $CategoryQuery.push({ [id]: value });
      } else {
        //When function is called from Chartquery it uses category label instead of wizard modal dropdowns
        value = $categorylabel[i][i];
        $CategoryQuery.push({ [id]: value });
      }
    }
  }
...
function next(divId) {
  ...
  if (itemDivId == "rpt-wiz-1") {
    ...
    if (boolPageResult && !SAVE_ITEMS) {
      ...
      CREATE_MODE ? fillFilterArrColumnsPage() : fillFilterEditMode();
    ...
    $("#textArea").val(ExportQuery());
    $OBJECT_ARRAY = OBJECT_ARRAY;
    categoryWizard("wizard");
    _Grouping_level = $(`#selectDataCategory`).val();
    _ReportSchema = $(`#selectReportSchema`).val();
    TEMPORARYCHART3.GroupingLevel = _Grouping_level;
    TEMPORARYCHART3.ReportSchema = _ReportSchema;
    $(`#myModalQuery`).remove();
  }

  if (boolPageResult) {
    if (itemDivId != "rpt-wiz-3")
      ...
      tmpBoolType
        ? ((FILTER_filterArr[index].input = "select"),
          (FILTER_filterArr[index].values = {
            0: dashResources.get("no"),
            1: dashResources.get("yes"),
          }),
          (tmpBoolType = false))
        : (tmpBoolType = false);
      if (tmpStringType) {
  ...
  if (OBJECT_ARRAY.length > 0) {
    ...
    OBJECT_ARRAY.map((item, index) => {
      ...
      if (Tmpselect) {
        if (
          (TmpoptionSort =
            Tmpselect.options[Tmpselect.selectedIndex] != undefined)
        )
      ...
      if (TmpselectOprator) {
        if (
          TmpselectOprator.options[TmpselectOprator.selectedIndex] != undefined
        )
          TmpCommandOprator =
            TmpselectOprator.options[TmpselectOprator.selectedIndex].value;
      }
    ...
    NEW_ARRAY_COLUMN.map((item, i) => {
      for (let index = 0; index < JsonData.RefColumns.length; index++) {
        if (item.Name == JsonData.RefColumns[index].Name) {
          ...
          item.Type = JsonData.RefColumns[index].Type.toLowerCase();
        }
      }
    });
    return true;
...
function ExportQuery() {
  ...
  if (boolPageCheck) {
    ...
    if (boolPageCheck) {
      ...
      boolPageCheck = false;
      itemDivId = "rpt-wiz-3";
      let SqlValidation = $("#builder-basic").queryBuilder(
        "getSQL",
        "question_mark"
      );
      ...
      if (boolPageCheck) {
        ...
        if (boolPageCheck) {
          ...
          if (boolPageCheck) {
            JsonData.Columns = NEW_ARRAY_COLUMN;
            ...
            OUTPUT[0].Filter = SqlValidation == null ? "" : JsonData.Filter;
            ...
            OUTPUT[0].WhereClause =
              SqlValidation == null ? "" : JsonData.WhereClause;
            $Query = {
              ChartID: $CurrentChart_ID,
              SelectClause: JsonData.SelectClause,
              Columns: JsonData.Columns,
              WhereClause: OUTPUT[0].WhereClause,
              Filter: OUTPUT[0].Filter,
            };

            //temporary save
            TEMPORARYCHART3.Query = $Query;

            let isGroup = JsonData.SelectClause;

            let Result;
            //is not WHERE
            if (SqlValidation == null) {
              Result = "" + JsonData.SelectClause;
            } else {
              //is WHERE with is GROUP BY
              if (isGroup.includes("GROUP BY")) {
                Result = isGroup
                  .slice(0, isGroup.indexOf(" GROUP BY"))
                  .concat("WHERE " + JsonData.WhereClause)
                  .concat(isGroup.slice(isGroup.indexOf(" GROUP BY")));
                //is not GROUP BY
              } else {
                Result = isGroup + " WHERE " + JsonData.WhereClause;
              }
            }
            if (Result != "") $(`#edit`).attr("disabled", false);
