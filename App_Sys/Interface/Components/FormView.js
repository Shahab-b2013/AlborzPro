...
function formView(
  ...
  var renderFileBrowse = function (itemObj, parentID) {
    ...
    try {
      ...
      var itemContent =
        ...
        itemObj.Name +
        `"><label ${
          itemObj.IsReadOnly === false ? "" : 'style = "display:none;"'
        } for="form-item-` +
        itemObj.FormItemID +
        '">' +
        itemObj.Label +
      ...
      if (itemObj.IsReadOnly == false) {
        ...
        let downloadHtml = `<input type="file" style="display:none;" class="form-control form-input file" data-show-upload="false" id="form-item-${itemObj.FormItemID}">`;
        ...
        if (itemObj.DefaultValue) {
          if (itemObj.AttributeTypeName !== "Image") {
            ...
            <div class="download-wrapper" style="display: flex; align-items: center; gap: 12px; margin-top: 8px;height: 70px;">
              <a href="#"
                 ...
                 data-activity-id="${itemObj.ActivityID}"
                 title="${$$Lang === "Fa" ? "دانلود" : "Download"}">
                <i class="fa fa-download" style="color:var(--A1)"></i>
                <span>${$$Lang === "Fa" ? "دانلود" : "Download"} ${
              itemObj.Label
            }</span>
              </a>
            </div>`;
            ...
            downloadHtml += `
            <div class="download-wrapper" style="margin-top: 10px;">
              <a href="${fileSrc}" download target="_blank"
          ...
          downloadHtml += `
            <div class="download-wrapper" style="display: flex; align-items: center; gap: 12px; margin-top: 8px;">
              <div class="fileDownloader" style="background-color:#a3a3a3; cursor: not-allowed;">
              <span>${
                $$Lang === "Fa" ? "فایلی بارگذاری نشده است" : "No File Uploaded"
              }</span>
              <i class="fa fa-ban"></i>
              </div>
            </div>`;
  ...
  var renderTableBox = function (itemObj, parentID, activityID) {
    ...
    if (_Rows.length) {
      ...
      for (let i = 0; i < _Rows.length; i++) {
        ...
        if (!itemObj.IsReadOnly) {
         ...
         <span id="Del_Row_${
           values[0].split("_")[1]
         }" class="btn remove glyphicon glyphicon-trash" onclick="Table_Del_Row(event ${
            _formItemId == 620029045 ? "," + _formItemId : ""
          })" style="font-size: 16px;line-height: inherit;"></span> 
         <span id="Edit_Row_${
           values[0].split("_")[1]
         }" class="btn glyphicon glyphicon-edit" onclick="Table_Edit_Row(event)"  style="font-size: 17px;line-height: normal;" ></span> 
          </td></tr>`;
    ...
    this.Table_Del_Row = function (e, formitemid) {
      ...
      let rowIndex = _obj.Rows.findIndex((x) => Object.values(x)[0] === RowID);
      if (formitemid == 620029045) {
        if (rowIndex > -1) {
          _obj.Rows[rowIndex]["ColumnState"] = "Deleted";
          _obj.Rows[rowIndex]["RowID"] = "0";
        }
        $(e.target).parent().parent().remove();
      } else {
        $(e.target).parent().parent().remove();
        if (rowIndex > -1) _obj.Rows.splice(rowIndex, 1);
      }
      let rows = $(`#${_tbody} tr`);
      ...
      let index = $TableData.findIndex((x) => x.ID === _obj.ID);
      if (index > -1) $TableData.splice(index, 1);
    ...
    function Add_Edit_Row(_formItemId, RowID, mode) {
      ...
      if (_formItemId == 620029045) {
        ...
        $("#item3").parent().parent().parent().css("display", "none");
            ...
            if (_type == "checkbox") {
              ...
              if (_type == "file") {
                ...
                if (
                  ...
                  if (_formItemId == 620029045 && _lbl === "ColumnState") {
                    _columnVal.push([_lbl, "Edited"]);
                  } else {
