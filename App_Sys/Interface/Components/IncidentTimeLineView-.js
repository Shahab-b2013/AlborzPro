...
function timeLineView(id, objKey) {
  ...
  this.renderContext = function (pageElementID) {
    ...
    $(bodyID)
      .append(`<ul class="nav nav-tabs" id="ul-1"><li id="tool-item-1" data-index="0" class="active" onclick="alert(1)">
        <a style="cursor:pointer" data-toggle="tab" class="tab-box-text">&nbsp;My Requests </a></li>
        <li id="tool-item-2" data-index="1" class="" onclick="alert(2)">
  ...
  this.renderModalContext = function (modalID) {
    ...
    $(bodyID).append(`<ul class="nav nav-tabs" id="ul-1">
       <li id="History-tab" data-index="0" class="tablinks active" onclick="tabOnActiion(event,'timeLine-${modalID}')">
        <a style="cursor:pointer" data-toggle="tab" class="tab-box-text">&nbsp;History </a></li>

         <li id="Alert-tab" data-index="1" class="tablinks" onclick="tabOnActiion(event,'Alert-${_objKey}')">
    ...
    if ($PForm.Name != "" && $PForm.TaskId != "" && $HistoryIds[objKey] != 6) {
      $("#timeLine-" + modalID).append(`&nbsp;&nbsp;<button type="button" id="btnAdd" onclick="openTModalContext(_modalID + 1, ` + $PForm.FormId + ',\'FormView\', [' + _objKey + '], 0, null, 0,' + $PForm.TaskId + `);" class="btn btn-edit" >Action <img alt="Action" src="App_Res/Images/Page/16/Edit.png"></button><br/><br/>`);
      $("#Alert-"+_objKey).append(`&nbsp;&nbsp;<button type="button" id="btnAdd" onclick="openTModalContext(_modalID + 1, ` + $PForm.FormId + ',\'FormView\', [' + _objKey + '], 0, null, 0,' + $PForm.TaskId + `);" class="btn btn-edit" >Action <img alt="Action" src="App_Res/Images/Page/16/Edit.png"></button><br/><br/>`);
    }
...
function tabOnActiion(evt, tabname) {
  ...
  if (tabname.split("-")[0] == "Alert") {
    ...
    let _detailData = new aData(6220003, null, _objKey, "");
    _detailData = _detailData.getList();

    // Alert
    Alerts(_data, _detailData, tabname, _objKey, 6220000);
  }
}
...
function showAlert(messageObj) {
  // // Success Form One Button:
  //   {title: 'Custom Alert!',text: 'This is a beautiful alert message.',icon: 'success',confirmButtonText: 'Close'}
  // // ErrorForm:
      ...
      str = `${date.getFullYear()}-${(date.getMonth() + 1)
        ...
        .padStart(2, "0")}-${date.getDate().toString().padStart(2, "0")} ${date
          ...
          .padStart(2, "0")}:${date
            .getMinutes()
            .toString()
            .padStart(2, "0")}:${date.getSeconds().toString().padStart(2, "0")} ${date.getHours() >= 12 ? "PM" : "AM"
        }`;
    }
  }
  return str;
...
function updateIncidentColor(color) {
  if (color.trim() !== "") {
    $.ajax({
      type: "POST",
      url: "../../App_Sys/Services/IncidentViewer.asmx/UpdateIncidentColor",
      data: JSON.stringify({
        incidentId: $("#IncidentIdText").val(),
        color: color,
      }),
      contentType: "application/json; charset=utf-8",
      success: function (response) {
        // در اینجا باید آیکن رنگ هندل بشه
        // location.reload();
      },
